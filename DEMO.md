...
This build is a **deployed technical Pilot — not a real-funds production launch**. Everything below runs on **Solana devnet with a test-USDC mint; no real funds are involved**. The 2026-07-15 identity-policy update supersedes the former invite-only boundary: Google and Apple registration are public, wallet login remains optional, and signup/onboarding do not request a personal wallet. The demo paths in this runbook remain operator/seed-prepared and are a superset of normal user access.

The user corridor is: **Google/Apple or wallet login → content create/upload (R2/Mux) → public feed/post projection → proposal intent → creator + sponsor dual sign → backend relay → manual Track 1 fixed-base settlement → campaign proof**. Social registration creates a platform-managed account and enters directly. A personal wallet is requested only when a future withdrawal/transfer action is explicitly initiated.

Closed for all Pilot users (seed/operator-only in this runbook, never a normal user path): custodial-to-personal withdrawal/transfer, unrestricted public managed execution, S1 market/buyout/portfolio claim, Track 2 endorsement and fan rewards, Track 3 CPS, daily/engagement rewards, automatic oracle settlement schedulers, and prototype/legacy routes.
...
- The Pilot-user corridor starts with Google/Apple or optional wallet login, then media → feed → proposal intent → dual sign → backend relay → manual Track 1 → campaign proof. Social signup does not request a personal wallet. S1, Track 2 endorsement, Track 3 CPS, withdrawals, rewards, and prototype routes remain closed.
