...
const CommentPanel = ({ post, variant = "sidebar", }) => (<aside className={`flex h-full flex-col ${variant === "sidebar"
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <link_1.default href={`/creators/${post.creatorId}`}>
            <img alt={post.creatorName} className="h-10 w-10 cursor-pointer rounded-full object-cover" src={post.creatorAvatarSrc}/>
          </link_1.default>
          <div>
    ...
    <div className="flex-1 overflow-y-auto px-5 py-5">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {post.tags.map((tag) => (<span className="text-xs text-[#85b9ff]" key={tag}>
            #{tag}
          </span>))}
      </div>
      ...
      <p className="mb-5 text-sm font-semibold text-white">共 {(0, mock_data_1.compactNumber)(post.commentsCount)} 条评论</p>
      <div className="space-y-5">
        {post.comments.map((comment) => (<CommentRow comment={comment} key={comment.id}/>))}
      </div>
    </div>
    ...
    <div className="border-t border-white/8 px-5 py-4">
      <div className="mb-3 flex items-center gap-5 px-1">
        <button className="flex items-center gap-1.5 text-sm text-[#c7d2e3] transition hover:text-white" type="button">
          ...
          <span>{(0, mock_data_1.compactNumber)(post.likes)}</span>
        </button>
        <button className="flex items-center gap-1.5 text-sm text-[#c7d2e3] transition hover:text-white" type="button">
          <span className="text-base">☆</span>
          <span>{(0, mock_data_1.compactNumber)(post.saves)}</span>
        </button>
        <button className="flex items-center gap-1.5 text-sm text-[#c7d2e3] transition hover:text-white" type="button">
          <span className="text-base">↗</span>
          <span>分享</span>
...
const CommentRow = ({ comment }) => (<div className="flex gap-3">
