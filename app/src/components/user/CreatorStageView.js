...
const shellCard = "app-shell-frame overflow-hidden border border-white/8 bg-[linear-gradient(180deg,rgba(16,23,34,0.94)_0%,rgba(12,18,28,0.92)_100%)]";
const CreatorStageView = ({ creator }) => {
    ...
    const [followPulse, setFollowPulse] = (0, react_1.useState)(false);
    const creatorPosts = (0, react_1.useMemo)(() => discover_1.posts.filter((post) => post.creatorId === creator.id), [creator.id]);
    ...
    return (<div className="space-y-6">
      <section className={shellCard}>
        <div className="relative h-52 overflow-hidden md:h-60">
          <ProgressiveImage_1.ProgressiveImage alt={`${creator.name} banner`} className="h-full w-full object-cover" fill priority sizes="(max-width: 768px) 100vw, 1200px" src={creator.heroSrc}/>
        ...
        <div className="relative px-6 pb-9 md:px-10">
          <div className="-mt-16 flex flex-col items-center text-center md:-mt-20">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
              <StagePill_1.StagePill stage={creator.state}/>
            ...
            <div className="mt-7 flex flex-wrap items-center justify-center gap-8 text-center">
              <ProfileStat label="关注" value={(0, utils_1.compactNumber)(creator.followingCount)}/>
              <div className="h-8 w-px bg-white/8"/>
              <ProfileStat label="粉丝" value={(0, utils_1.compactNumber)(creator.followersCount)}/>
              <div className="h-8 w-px bg-white/8"/>
              <ProfileStat label="获赞与收藏" value={(0, utils_1.compactNumber)(creator.totalLikesAndSavesCount)}/>
            </div>
            ...
            <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
              ...
              <button className={`rounded-full px-7 py-2.5 text-sm font-semibold transition hover:bg-[#ea523e] ${isFollowing ? "bg-[#13291f] text-[#90efac]" : "bg-[#de402a] text-white"} ${followPulse ? "tap-bounce-active" : ""}`} onClick={toggleFollow} type="button">
      ...
      <section className={`${shellCard} px-6 py-6 md:px-8`}>
        <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          ...
          <button className="rounded-full bg-[#de402a] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#ea523e]" type="button">
            {getPrimaryActionLabel(creator)}
          </button>
        </div>

        <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
          {metrics.map((metric) => (<div className="glass-card p-4" key={metric.label}>
      ...
      <section className={shellCard}>
        <div className="border-b border-white/8 px-6 pt-5 md:px-8">
          <div className="flex items-center gap-7">
            {["作品", "投资档案", "Signals"].map((tab) => (<button className={`relative pb-4 text-sm transition ${activeTab === tab ? "font-semibold text-white" : "text-[#7f90aa] hover:text-white"}`} key={tab} onClick={() => setActiveTab(tab)} type="button">
                {tab}
        ...
        <div className="px-6 py-6 md:px-8">
          {activeTab === "作品" ? (<div className="masonry-grid">
              {creatorPosts.map((post) => (<link_1.default className="block" href={`/posts/${post.id}`} key={post.id}>
                  <div className="glass-card">
                    <div className={`relative overflow-hidden rounded-t-[24px] ${post.mediaHeightClass}`}>
                      <ProgressiveImage_1.ProgressiveImage alt={post.title} className="object-cover transition-transform duration-500 hover:scale-[1.02]" fill sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" src={post.coverSrc}/>
                      <div className="absolute inset-0 bg-gradient-to-t from-black/36 to-transparent"/>
                      {post.type === "VIDEO" ? (<div className="liquid-pill absolute right-3 top-3 rounded-full px-2.5 py-1 text-[10px] uppercase tracking-[0.16em] text-white">
                          Video
                        </div>) : null}
                    </div>
                    ...
                    <div className="glass-card-footer px-4 pb-4 pt-4">
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        ...
                        <span className="text-xs text-[#8ea0ba]">♡ {(0, utils_1.compactNumber)(post.likes)}</span>
                      </div>
                    </div>
                  </div>
                </link_1.default>))}
...
const ProfileStat = ({ label, value }) => (<div className="min-w-[92px]">
...
const InvestmentPanels = ({ creator }) => {
    if (creator.state === "S1_DISCOVERY") {
        ...
        <div className="space-y-4">
          <InfoCard eyebrow="Graduation watch" title={`${creator.graduationProgress}% of graduation target`} body={`Current price is ${(0, utils_1.formatUsd)(creator.tokenPrice)} with a graduation target at ${(0, utils_1.formatUsd)(creator.targetGraduationPrice)}.`}/>
    ...
    if (creator.state === "S1_BUYOUT") {
        ...
        <InfoCard eyebrow="Supporter outcome" title={`${(0, utils_1.formatUsd)(creator.supporterDistributableUsd ?? 0)} distributable to supporters`} body="The profile should explain payout timing, window state, and what supporters receive when settlement closes."/>
    ...
    return (<div className="grid gap-4 lg:grid-cols-[1.1fr_1fr]">
      ...
      <InfoCard eyebrow="Execution state" title={`${creator.activeCampaignCount ?? 0} active campaigns · activity score ${creator.activityScore ?? 0}`} body="This creator has moved into S2 and should read like an operating profile layered on top of a creator profile."/>
    </div>);
};
...
const InfoCard = ({ eyebrow, title, body }) => (<div className="glass-card p-5">
...
const getInvestmentMetrics = (creator) => {
    if (creator.state === "S1_DISCOVERY") {
        return [
            { label: "Current price", value: (0, utils_1.formatUsd)(creator.tokenPrice), tone: "text-[#8ed0ff]", help: undefined },
            { label: "Holders", value: (0, utils_1.compactNumber)(creator.holderCount), tone: "text-white", help: undefined },
            { label: "Graduation", value: `${creator.graduationProgress}%`, tone: "text-white", help: "distance to graduation" },
            { label: "Target price", value: (0, utils_1.formatUsd)(creator.targetGraduationPrice), tone: "text-white", help: undefined },
        ];
    }
    if (creator.state === "S1_BUYOUT") {
        return [
            { label: "Current price", value: (0, utils_1.formatUsd)(creator.tokenPrice), tone: "text-[#8ed0ff]", help: undefined },
            { label: "Supporter pool", value: (0, utils_1.formatUsd)(creator.supporterDistributableUsd ?? 0), tone: "text-white", help: "distributable after close" },
            { label: "Offer value", value: (0, utils_1.formatUsd)(creator.buyoutOfferUsd ?? 0), tone: "text-white", help: undefined },
            { label: "Holders", value: (0, utils_1.compactNumber)(creator.holderCount), tone: "text-white", help: undefined },
        ];
    }
    return [
        ...
        { label: "Valuation", value: (0, utils_1.formatUsd)(creator.valuationUsd ?? 0), tone: "text-white", help: undefined },
    ];
};
