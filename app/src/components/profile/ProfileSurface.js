...
exports.resolveProfilePosts = exports.resolveItemPostId = exports.ProfileNoteGrid = exports.ProfileTabBar = exports.ProfileHero = exports.PROFILE_TABS = void 0;
const StagePill_1 = require("@/components/shared/StagePill");
const utils_1 = require("@/lib/mocks/utils");
exports.PROFILE_TABS = ["笔记", "收藏", "点赞"];
const ProfileHero = ({ avatarSrc, bannerSrc, bio, followersCount, followingCount, handle, likesAndSavesCount, location, name, }) => (<section className="-mx-4 border-b border-white/8 bg-[#0a0f18] lg:-mx-6">
...
const ProfileTabBar = ({ activeTab, onTabChange, }) => (<section className="mx-auto max-w-[960px] px-2 pt-8">
    <div className="mx-auto flex max-w-[560px] items-center justify-center gap-8 border-b border-white/8">
      {exports.PROFILE_TABS.map((tab) => (<button className={`relative pb-4 text-sm transition ${activeTab === tab ? "font-semibold text-white" : "text-[#7d8da6] hover:text-white"}`} key={tab} onClick={() => onTabChange(tab)} type="button">
          {tab}
          {activeTab === tab ? <span className="absolute inset-x-0 bottom-0 h-0.5 rounded-full bg-[#de402a]"/> : null}
        </button>))}
    </div>
  </section>);
exports.ProfileTabBar = ProfileTabBar;
const ProfileNoteGrid = ({ items, onOpen, }) => (<section className="mx-auto max-w-[960px] px-2 pt-2">
    <div className="masonry-grid">
      {items.map((item) => (<ProfileNoteCard item={item} key={item.id} onOpen={() => onOpen(item)}/>))}
    </div>
  </section>);
exports.ProfileNoteGrid = ProfileNoteGrid;
const resolveItemPostId = (item) => {
...
exports.resolveItemPostId = resolveItemPostId;
const resolveProfilePosts = (items, posts) => {
    const postIndex = new Map(posts.map((post) => [post.id, post]));
    const seen = new Set();
    const resolved = [];
    for (const item of items) {
        const postId = (0, exports.resolveItemPostId)(item);
...
exports.resolveProfilePosts = resolveProfilePosts;
const Stat = ({ label, value }) => (<div className="min-w-[110px] px-6">
    <p className="text-[28px] font-semibold tracking-[-0.04em] text-white">{value}</p>
    <p className="mt-1 text-xs text-[#8797ae]">{label}</p>
  </div>);
const ProfileNoteCard = ({ item, onOpen, }) => (<button className="block w-full text-left" onClick={onOpen} type="button">
    <div className="card-radius overflow-hidden border border-white/[0.06] bg-[#101621] shadow-[0_16px_44px_rgba(0,0,0,0.22)] transition hover:-translate-y-0.5 hover:border-white/[0.1]">
      ...
      <div className="px-3.5 pb-3.5 pt-3">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <span className="text-xs text-[#8ea0ba]">♡ {(0, utils_1.compactNumber)(item.likes)}</span>
        </div>
      </div>
    </div>
  </button>);
