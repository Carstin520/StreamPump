  ...
  return (
    <div className="mt-3 rounded-[14px] border border-white/[0.07] bg-white/[0.025] p-3.5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[length:var(--fs-micro)] font-semibold uppercase tracking-[0.16em] text-[#8ea0ba]">{title}</p>
          <p className="mt-1 text-[length:var(--fs-overline)] leading-relaxed text-[#cbd6e7]">{description}</p>
          {amountLabel ? (
            <p className="mt-1 font-mono text-[length:var(--fs-micro)] text-[#67b8ff]">{amountLabel}</p>
          ) : null}
        </div>
      ...
      {state.status === "success" ? (
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
          <p className="text-[length:var(--fs-overline)] font-semibold text-[#65ecaf]">{successLabel}</p>
          <button
            className="rounded-full border border-white/[0.08] px-3 py-1.5 text-[length:var(--fs-micro)] font-medium text-[#cbd6e7] transition hover:text-white"
            onClick={onCancel}
            type="button"
          >
            Done
