...
import { compactNumber, findPost, posts } from "@/lib/mock-data";
    ...
    <main className="min-h-screen bg-[#090d14] text-white">
      <section className="grid min-h-screen grid-cols-1 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="relative flex items-center justify-center bg-black/50 p-6 xl:p-8">
          <Link
            className="absolute left-6 top-6 z-20 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-black/35 text-lg text-white backdrop-blur-md transition hover:bg-black/55"
            href="/explore"
          >
            ←
          </Link>
          ...
          <div className="relative w-full max-w-[620px]">
            <img
              alt={post.title}
              className="max-h-[84vh] w-full rounded-[28px] object-contain shadow-[0_34px_90px_rgba(0,0,0,0.45)]"
            ...
            {images.length > 1 ? (
              <>
                {currentImage > 0 ? (
                  <button
                    className="absolute left-3 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-md transition hover:bg-black/65"
                    onClick={goPrev}
                    type="button"
                  >
                    ‹
                ...
                {currentImage < images.length - 1 ? (
                  <button
                    className="absolute right-3 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-md transition hover:bg-black/65"
                    onClick={goNext}
                    type="button"
                  >
                    ›
                    ...
                    {images.map((_, index) => (
                      <button
                        className={`rounded-full transition-all ${
                          index === currentImage ? "h-1.5 w-4 bg-white" : "h-1.5 w-1.5 bg-white/45 hover:bg-white/70"
                        }`}
                        key={`${post.id}-${index}`}
...
const VideoPostView = () => {
  ...
  const currentIndex = Math.max(
    0,
    videoPosts.findIndex((item) => item.id === postId),
  );
  ...
  return (
    <main className="fixed inset-0 z-50 overflow-hidden bg-black text-white">
      <Link
        className="absolute left-5 top-5 z-40 flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-black/45 text-lg backdrop-blur-md transition hover:bg-black/65"
        href="/explore"
      >
        ←
      </Link>
      ...
      <div className="absolute right-5 top-5 z-40 flex flex-col items-center gap-2">
        <Link
          className={`flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-md transition ${previousPost ? "hover:bg-black/65" : "pointer-events-none opacity-30"}`}
        ...
        <Link
          className={`flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-black/45 text-white backdrop-blur-md transition ${nextPost ? "hover:bg-black/65" : "pointer-events-none opacity-30"}`}
        ...
        {videoPosts.map((item, index) => (
          <div
            className={`w-1 rounded-full transition-all duration-300 ${
              item.id === post.id ? "h-6 bg-white" : "h-1.5 bg-white/30"
            }`}
            key={`${item.id}-${index}`}
          />
        ))}
        ...
        <div className="relative flex-1 overflow-hidden">
          ...
          <div className="absolute bottom-24 right-4 z-20 flex flex-col gap-5 items-center lg:right-6">
            {[
              ...
              `☆ ${compactNumber(post.saves)}`,
              "↗ 分享",
            ].map((label, index) => (
              <button
                className="flex flex-col items-center gap-1 text-white"
                key={`${post.id}-${index}-${label}`}
                onClick={() => {
                  if (label.includes("💬")) setShowComments((value) => !value);
                }}
                type="button"
              >
