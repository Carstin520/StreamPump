...
import { EarningsConsole } from "@/components/workspace/EarningsConsole";
import {
  ConsoleAuthRequired,
  ConsoleLoading,
...
export default function WorkspaceEarningsPage() {
  ...
  useEffect(() => {
    ...
    if (!session) {
      setState({ status: "unauthenticated", loginHref: buildLoginHref({ nextPath: WORKSPACE_EARNINGS_PATH }) });
      return;
    }
      ...
      .catch((error: unknown) => {
        ...
        if (isAuthError(error)) {
          setState({
            ...
            loginHref: clearAuthAndBuildLoginHref(WORKSPACE_EARNINGS_PATH),
          });
          return;
        }
        setState({
  ...
  return (
    <>
      <Head>
        <title>{t("ws.earnings.pageTitle")}</title>
      </Head>
      <WorkspaceShell stage={persona.stage} wallet={persona.wallet}>
        <ProductReadinessBanner
          description={t("ws.earnings.readinessDesc")}
          status="MOCK_PREVIEW"
          title={t("ws.earnings.readinessTitle")}
        />
        ...
        {(isDemoMode || state.status === "ready" || state.status === "preview") && <EarningsConsole />}
      </WorkspaceShell>
    </>
  );
}
