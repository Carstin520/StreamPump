...
exports.default = ManifestDetailPage;
...
const resolveAssetType = (file) => file.type.toLowerCase().startsWith("video/") ? "VIDEO" : "IMAGE";
...
function ManifestDetailPage() {
    ...
    const [publicationPlatform, setPublicationPlatform] = (0, react_1.useState)("XIAOHONGSHU");
    const [publicationUrl, setPublicationUrl] = (0, react_1.useState)("");
    const [publicationPostId, setPublicationPostId] = (0, react_1.useState)("");
    const refreshManifest = async (token, manifestId) => {
        const data = await (0, workspace_1.getContentManifestById)(token, manifestId);
    ...
    (0, react_1.useEffect)(() => {
        ...
        void (0, workspace_1.getContentManifestById)(session.accessToken, manifestId)
            .then((data) => {
            if (!cancelled) {
                setState({ kind: "ready", data });
            }
    ...
    const handleUploadAssets = async () => {
        ...
        try {
            ...
            const inputs = await Promise.all(selectedFiles.map(async (file, index) => ({
            ...
            const response = await (0, workspace_1.presignManifestAssets)(session.accessToken, manifestId, inputs);
            for (const [index, upload] of response.uploads.entries()) {
                ...
                await (0, workspace_1.completeManifestAssetUpload)(session.accessToken, manifestId, upload.assetId);
            }
    ...
    const handleFinalize = async () => {
        ...
        try {
            ...
            const result = await (0, workspace_1.finalizeContentManifest)(session.accessToken, manifestId);
            await refreshManifest(session.accessToken, manifestId);
            setActionMessage(result.manifestHashHex
    ...
    const handleCreatePublication = async () => {
        ...
        try {
            ...
            await (0, workspace_1.createContentPublication)(session.accessToken, {
                manifestId,
    ...
    return (<>
      ...
      <WorkspaceShell_1.WorkspaceShell subtitle="This view is where upload, processing, and finalize status need to feel operationally clear without looking like an ops dashboard." title={manifestTitle}>
        {state.kind === "loading" ? <AsyncStateCard_1.AsyncStateCard body="Loading manifest detail, assets, and publication records from the v1 content API." title="Loading manifest"/> : null}
        {state.kind === "auth" ? <AsyncStateCard_1.AsyncStateCard actionHref="/login" actionLabel="Open login" body="Manifest detail now uses authenticated content APIs. Sign in to load creator-owned manifest data." title="Session required"/> : null}
        {state.kind === "error" ? <AsyncStateCard_1.AsyncStateCard body={state.message} title="Manifest request failed"/> : null}
        {state.kind === "ready" ? (<div className="grid gap-5 xl:grid-cols-[1fr_340px]">
            <div className="space-y-5">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-300">Owner: {(0, formatting_1.shortenWallet)(state.data.creatorWallet)} · {state.data.contentType}</p>
                    </div>
                    <span className="rounded-full bg-white/12 px-3 py-1 text-xs text-slate-100">{(0, formatting_1.formatIsoLabel)(state.data.updatedAt)}</span>
                  </div>
                  <div className="grid gap-3 md:grid-cols-3">
                    {state.data.assets.map((asset) => (<div className="rounded-3xl border border-dashed border-white/10 bg-white/4 p-4" key={asset.assetId}>
                    ...
                    {state.data.assets.length === 0 ? (<div className="rounded-3xl border border-dashed border-white/10 bg-white/4 p-4 text-sm text-slate-300 md:col-span-3">
              ...
              <section className="glass-card p-5">
                <div className="space-y-4">
                  ...
                  <input accept=".mp4,.mov,.jpg,.jpeg,.png,.webp,.heic,video/mp4,video/quicktime,image/jpeg,image/png,image/webp,image/heic" className="block w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200 file:mr-4 file:rounded-full file:border-0 file:bg-white file:px-4 file:py-2 file:text-sm file:font-medium file:text-slate-950" multiple onChange={handleFileSelection} type="file"/>
                  {selectedFiles.length > 0 ? (<div className="rounded-2xl bg-white/5 p-4">
                      <p className="text-xs uppercase tracking-[0.18em] text-slate-400">Pending upload set</p>
                      <div className="mt-3 space-y-2">
                        {selectedFiles.map((file) => (<div className="flex items-center justify-between gap-3 text-sm text-slate-200" key={`${file.name}-${file.size}-${file.lastModified}`}>
                  ...
                  <div className="flex gap-3">
                    <button className="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-950 disabled:cursor-wait disabled:opacity-70" disabled={busyAction !== null} onClick={() => void handleUploadAssets()} type="button">
                      Upload selected assets
                    </button>
                    <button className="rounded-full border border-white/12 px-4 py-2 text-sm text-slate-200 disabled:opacity-70" disabled={busyAction !== null || selectedFiles.length === 0} onClick={() => {
                setSelectedFiles([]);
              ...
              <section className="glass-card p-5">
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <button className="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-950 disabled:cursor-wait disabled:opacity-70" disabled={busyAction !== null} onClick={() => void handleFinalize()} type="button">
                      Finalize manifest
                    </button>
                  </div>
            ...
            <section className="glass-card p-5">
              <div className="space-y-4">
                ...
                {state.data.publications.length > 0 ? (<div className="space-y-3">
                    {state.data.publications.map((publication) => (<div className="rounded-2xl bg-white/5 p-4" key={publication.publicationId}>
                  ...
                  </div>) : (<p className="text-sm leading-7 text-slate-300">No publication URLs are linked yet. Once a platform post exists, create a publication record here so the manifest can move into the published lifecycle.</p>)}
                ...
                <div className="rounded-3xl border border-white/10 bg-white/4 p-4">
                  <div className="space-y-3">
                    ...
                    <label className="space-y-2">
                      ...
                      <input className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none" onChange={(event) => setPublicationPlatform(event.target.value.toUpperCase())} value={publicationPlatform}/>
                    </label>
                    <label className="space-y-2">
                      <span className="text-xs uppercase tracking-[0.18em] text-slate-400">External URL</span>
                      <input className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none" onChange={(event) => setPublicationUrl(event.target.value)} placeholder="https://example.com/post/123" value={publicationUrl}/>
                    </label>
                    <label className="space-y-2">
                      <span className="text-xs uppercase tracking-[0.18em] text-slate-400">External post id (optional)</span>
                      <input className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white outline-none" onChange={(event) => setPublicationPostId(event.target.value)} placeholder="post-123" value={publicationPostId}/>
                    </label>
                    <button className="rounded-full bg-white px-4 py-2 text-sm font-medium text-slate-950 disabled:cursor-wait disabled:opacity-70" disabled={busyAction !== null} onClick={() => void handleCreatePublication()} type="button">
