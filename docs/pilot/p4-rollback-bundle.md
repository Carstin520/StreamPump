...
M4 and M5 completed on 2026-07-13 as controlled technical Pilot verification. The M4 Render deploy was `dep-d9ac42daeets73djf58g` and the M4 Vercel Production deployment was `dpl_26s2wP8KGqGJQbVeJH5VvXo2GmK2`; both were at the exact commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. After M5's final Mux secret rotation the **current** deployments are Render `dep-d9adr95aeets73dlmltg` on service `srv-d79rs0450q8c73fp2lmg` and Vercel Production `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` (Ready), both at that same exact commit; auto-deploy is disabled and pre-deploy is the read-only `npm run verify:p4:neon:post` exact-26 gate. The application rollback target remains exactly commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`: the missing-signature 500 controller fix is not deployed, so an honest 097e rollback restores the missing-signature 500 behavior. Preserve the frozen pre-M4 targets in the table above, but do not automatically restore the old Render binary against migrated Neon: first stop traffic and obtain an explicit recovery decision because it predates the P2/P3 schema. Vercel can be rolled back independently to `dpl_DmwV2BsLVjmS2ifqCDat9hQpAETV` if the frontend promotion alone fails. A code rollback does not reverse a migration; the Neon restore/repoint decision is separate and must be explicit.
...
- **Application rollback restores 097e behavior honestly.** Until the missing-signature controller patch is deployed, the application rollback target is exactly commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`, which rejects an invalid present `mux-signature` with 401 but returns 500 for a missing signature. Rolling back honestly restores that missing-signature 500 behavior; do not describe the rollback as carrying the undeployed 401 fix.
