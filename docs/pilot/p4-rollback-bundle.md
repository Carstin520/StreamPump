...
Allowlist cleanup completed: the sole entry is `GYjkMEZEFHuY4uRZVwE79eeXAGtoneh53gb49X4HqCMH`; both disposable actors are absent. Their locally verified signatures received `401 AUTH_CHALLENGE_INVALID`, the controller's intentional concealment of `PILOT_INVITE_REQUIRED`. Keep invite-only and automated settlement off. If application rollback is required, `67ec60c` remains the last known-good code artifact, but pair it with the approved baseline allowlist rather than restoring disposable actors, and set `PILOT_EXPECTED_RELEASE_SHA` to the rollback commit before deployment.
