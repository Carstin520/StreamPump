...
Gate status: **M2, M3, M4, and M5 completed successfully as controlled technical Pilot verification. The live full padded program hash is `a6008d9c11304c73324db9f5645ccd4e303015f0e0f03671f3d41fd42a720732`; chain rollback was not required. Neon production has 26 applied migrations and current-point recovery branch `br-frosty-fire-an0lsiq2` is retained unchanged. Render service `srv-d79rs0450q8c73fp2lmg` deploy `dep-d9aed2t8nd3s73au07qg` is live at backend commit `73df4e2c7a6367b5b28871510d8ced095e59be6c` (the hardened Mux webhook authentication); Vercel Production `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` (Ready) remains at frontend commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19` because this change was backend-only. The backend application rollback target is exactly `73df4e2c7a6367b5b28871510d8ced095e59be6c` with a matching `PILOT_EXPECTED_RELEASE_SHA`; Vercel Production remains unchanged at frontend commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. This is an application-behavior rollback only: `73df4e2c…` currently serves `/ready` HTTP 503 (indexer `FAILED`), so redeploying it restores prior backend behavior but does **not** recover indexer/`/ready` readiness. M6 is approved but not mutated; it remains a separate human gate.**

**Current live readiness (blocking M6):** the live `73df4e2c…` backend now returns `/health` HTTP 200 but `/ready` HTTP 503 — `database: READY`, `indexer: FAILED`, `muxReconciliation: READY` — because the deploy has no working WebSocket slot subscription (`SOLANA_INDEXER_WS_ENDPOINT` unset/non-matching; the Alchemy-derived WS path returns `-32601` for `slotSubscribe`/`logsSubscribe`). Hosted validation now requires `SOLANA_INDEXER_WS_ENDPOINT` to correspond exactly to `SOLANA_INDEXER_RPC_ENDPOINT` (identical host, port, path, and query; only the `https`→`wss` scheme differs), and the provider dashboard must supply that matching pair — a non-matching pair is a stop. All deploy and chain mutations remain stopped. A `73df4e2c…` rollback does not by itself restore indexer readiness. Separately, the Fable 5 review of candidate commit `80be8eb` returned **FAIL** (0 blocker / 1 major / 1 minor); those findings are now closed in the follow-up worktree, where full backend tests pass 225 alongside the M6 actor-helper, deployment-verifier, and backend build. The newly frozen follow-up HEAD still requires push, CI, and a fresh Fable 5 PASS before any deploy — do not claim PASS.
...
M4 and M5 completed on 2026-07-13 as controlled technical Pilot verification. The M4 Render deploy was `dep-d9ac42daeets73djf58g` and the M4 Vercel Production deployment was `dpl_26s2wP8KGqGJQbVeJH5VvXo2GmK2`; both were at the exact commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`, and an interim M5 Render deploy was `dep-d9adr95aeets73dlmltg`. After the M5 Mux webhook-authentication hardening the **current** Render deployment is `dep-d9aed2t8nd3s73au07qg` on service `srv-d79rs0450q8c73fp2lmg`, live at the **backend** commit `73df4e2c7a6367b5b28871510d8ced095e59be6c`; the **current** Vercel Production deployment remains `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` (Ready) at the **frontend** commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19` (not redeployed — backend-only change). Auto-deploy is disabled and pre-deploy is the read-only `npm run verify:p4:neon:post` exact-26 gate. Post-deploy checks against the live backend passed: `/health` HTTP 200 with mode `INVITE_ONLY_PILOT`, `automatedSettlement` false, invite-only access configured; `/ready` HTTP 200 `READY`; a signed-but-ignored Mux event 200, a missing signature 401, an invalid signature 401; and the current Vercel `/explore` HTTP 200 with neither `Service Suspended` nor `Feed unavailable`. The application rollback target remains exactly commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`: because the deployed missing-signature 401 fix lives in backend commit `73df4e2c…`, an honest 097e rollback restores the prior missing-signature 500 behavior. Preserve the frozen pre-M4 targets in the table above, but do not automatically restore the old Render binary against migrated Neon: first stop traffic and obtain an explicit recovery decision because it predates the P2/P3 schema. Vercel can be rolled back independently to `dpl_DmwV2BsLVjmS2ifqCDat9hQpAETV` if the frontend promotion alone fails. A code rollback does not reverse a migration; the Neon restore/repoint decision is separate and must be explicit.
...
- **Application rollback restores 097e behavior honestly.** The missing-signature controller patch is now deployed at backend commit `73df4e2c7a6367b5b28871510d8ced095e59be6c` (Render `dep-d9aed2t8nd3s73au07qg`), where the live backend returns 401 for both a missing and an invalid `mux-signature`. The application rollback target nonetheless remains exactly commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`, which rejects an invalid present `mux-signature` with 401 but returns 500 for a missing signature. Rolling back honestly restores that missing-signature 500 behavior; do not describe the rollback as carrying the deployed 401 fix.
