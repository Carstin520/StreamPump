...
Gate status: **M2–M6 completed within the technical Pilot boundary. Render deploy `dep-d9auio7lk1mc73c4r18g` is live at backend `88c0debad6ecb7eacfe9e24793951f3794353f4c`; Vercel remains `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` at frontend `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. `/health` and `/ready` are healthy with automated settlement off. The sole allowlist entry is the human-approved external wallet `GYjkMEZEFHuY4uRZVwE79eeXAGtoneh53gb49X4HqCMH`; disposable actors are removed. The M6 payout is irreversible. H4 is ready for human review, not approved.**

**Current live readiness:** release `88c0deb…` returns `/health` HTTP 200 with `INVITE_ONLY_PILOT` and `automatedSettlement=false`; `/ready` is `READY`. Existing M6 ranges and the runtime-gate closure review are complete; do not repeat them.
...
M4–M6 deployment identifiers remain historical evidence. Current Render deployment `dep-d9auio7lk1mc73c4r18g` on service `srv-d79rs0450q8c73fp2lmg` is live at `88c0debad6ecb7eacfe9e24793951f3794353f4c`; Vercel Production remains `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` at `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. Auto-deploy is disabled and pre-deploy is the read-only exact-26 runtime Neon gate. The M6 commit `67ec60c7679aca2d7adad24780ae043370c426e3` is retained only as historical, superseded execution evidence and must not be used for a post-H6 application rollback. After P5/P6, the current application deployment and rollback truth is [`p6-final-audit-and-release-handoff.md`](./p6-final-audit-and-release-handoff.md), and the only current application rollback target is backend `88c0debad6ecb7eacfe9e24793951f3794353f4c` at Render deployment `dep-d9auio7lk1mc73c4r18g`. A post-H6 rollback must restore the previously preserved Render environment revision, set `PILOT_EXPECTED_RELEASE_SHA` to that full backend SHA, verify only exact release identity plus `/health` and `/ready`, and then stop without any downstream mutation. Never redeploy `5d07748…` or `73df4e2c…` as routine rollback because they contain known failures. No application rollback reverses Neon migrations, the program upgrade, creator registration/receipt, test-USDC state, proposal funding, or the finalized Track 1 payout.
...
Allowlist cleanup completed: the sole entry is `GYjkMEZEFHuY4uRZVwE79eeXAGtoneh53gb49X4HqCMH`; both disposable actors are absent. Their locally verified signatures received `401 AUTH_CHALLENGE_INVALID`, the controller's intentional concealment of `PILOT_INVITE_REQUIRED`. Keep invite-only and automated settlement off. The M6 commit `67ec60c7679aca2d7adad24780ae043370c426e3` remains historical evidence only; it is superseded and prohibited as a post-H6 application rollback target. Follow the current [P6 application rollback contract](./p6-final-audit-and-release-handoff.md): restore the preserved Render environment revision, redeploy only `88c0debad6ecb7eacfe9e24793951f3794353f4c` (`dep-d9auio7lk1mc73c4r18g`), bind `PILOT_EXPECTED_RELEASE_SHA` to that full SHA, verify exact release identity plus `/health` and `/ready`, and stop without restoring disposable actors or continuing downstream mutation.
...
- Disposable actor public keys, minimal funding amounts, proposal PDA, manifest ID/hash, exact Track 1 settlement/replay signature, exactly-once balance result, approved baseline public key, and allowlist cleanup status.
