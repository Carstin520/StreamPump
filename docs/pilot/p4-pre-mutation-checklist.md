...
Gate status: **M2–M5 completed successfully. M6 completed through the disposable external-wallet corridor, natural-deadline manual Track 1 settlement, and same-idempotency-key replay. The live devnet program remains at capacity 1,328,344 with padded SHA256 `a6008d9c11304c73324db9f5645ccd4e303015f0e0f03671f3d41fd42a720732`; Neon has exactly 26 applied migrations and the recovery branch is retained. Render deploy `dep-d9at6enavr4c73b0rc20` is live at backend commit `67ec60c7679aca2d7adad24780ae043370c426e3`; Vercel Production remains `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` at frontend commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. This remains an invite-only, external-wallet-first, Solana devnet/test-USDC, Track1-only controlled technical Pilot with no real funds — not a formal production launch. Allowlist restoration is blocked pending an approved non-disposable external Pilot wallet, so H4 is pending.**

**Current live readiness:** `/health` is HTTP 200 with `INVITE_ONLY_PILOT`, `automatedSettlement=false`, and release `67ec60c7679aca2d7adad24780ae043370c426e3`; `/ready` is HTTP 200 with database/indexer/Mux reconciliation all `READY`. GitHub checks and `pilot-chain` passed. Three contiguous exact Fable ranges (`80be8eb..a1de424`, `a1de424..5d07748`, `5d07748..67ec60c`) cover the frozen candidate and each passed with zero blocker/major; do not repeat a full-repository review.
...
M4 and M5 were human-approved in sequence and both passed as controlled technical Pilot verification. M6 was subsequently approved and executed through settlement/replay; its remaining cleanup blocker is recorded below.
...
**Current deployed state:** Render deployment `dep-d9at6enavr4c73b0rc20` on service `srv-d79rs0450q8c73fp2lmg` is live at backend commit `67ec60c7679aca2d7adad24780ae043370c426e3`. Vercel Production remains `dpl_6f9LBgHRqB8hCywV5DimXfV9YqUK` (Ready) at frontend commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`; M6 did not redeploy Vercel. Earlier M4/M5 deployment identifiers remain historical evidence in the rollback bundle.

Candidate SHA history: the prior pre-doc candidate `aa59485902194af6132e430ab1c53f2c0c931038` is not remote-reachable and was not deployed. M4 deployed `097e9805b197398ae1c04cf5bf84f1044b3b2f19` to Render and Vercel; M5 later advanced only Render to `73df4e2c…`; M6 has now advanced only Render to current `67ec60c7679aca2d7adad24780ae043370c426e3`. Vercel remains at `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. Auto-deploy remains disabled/controlled so neither `main` nor an unpinned branch head is deployed.
...
- [ ] Restore a non-disposable invite baseline and prove both disposable actors receive `403`. **Blocked:** no pre-M6 non-disposable baseline exists; removing both current entries would empty the fail-closed allowlist. Human must supply or approve at least one legitimate non-disposable external Pilot wallet public key. Never use the fee payer/admin/oracle or infer a wallet from DB data.
...
H4 is **pending** until the M6 allowlist cleanup above is completed and verified. P4 evidence does not authorize P5/P6, public launch, real funds, readiness promotion, external-wallet expansion, or any closed Pilot lane.
