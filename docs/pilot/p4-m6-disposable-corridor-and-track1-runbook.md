...
1. **Commit and push the fixed M6 candidate.** Commit the exact M6 corridor/settlement code on `codex/p4-pilot-deployment` and push it so a complete 40-hex commit SHA is remote-reachable. Auto-deploy stays disabled; nothing deploys from an unpinned branch head.
2. **One Fable 5 release-review gate.** Run a single Fable 5 review of the pushed candidate diff as the release gate and record its verdict in the evidence bundle. Do not proceed on an unreviewed candidate.
3. **Pin and deploy the exact commit on Render.** Set the Render env `PILOT_EXPECTED_RELEASE_SHA` to the fixed candidate SHA (the backend fails startup unless it exactly equals `RENDER_GIT_COMMIT`) and deploy that exact commit to service `srv-d79rs0450q8c73fp2lmg`. **Vercel Production stays unchanged** at frontend commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19` (M6 is backend-only).
4. **Verify the live release before any mutation.** Confirm `/health` HTTP 200 with mode `INVITE_ONLY_PILOT`, `automatedSettlement: false`, invite-only access configured, and `health.releaseSha` equal to the candidate SHA; `/ready` HTTP 200 `READY` with database/indexer/muxReconciliation all `READY`. `npm run verify:p4:deployment` performs these `/health`+`/ready`+release checks; pass matching `--expected-release-sha` and `--deployed-release-sha` (both the candidate SHA).
5. **Hold the rollback path ready.** The application rollback target is backend commit `73df4e2c7a6367b5b28871510d8ced095e59be6c` (the current M5 deploy). Rolling back means redeploying that exact commit **and** resetting `PILOT_EXPECTED_RELEASE_SHA` to match it so the env SHA and deployed SHA stay identical; Vercel is not touched. An honest 73df4e2c rollback restores the prior backend behavior only.

- **Expected:** the live backend serves the fixed candidate SHA on `/health`, `/ready` is fully `READY`, and the rollback commit + matching env SHA are staged.
- **Fail-closed stop:** any release-SHA mismatch, a non-`READY` service, a Vercel change, or an unreviewed/unpushed candidate. Do not start actor preparation.
