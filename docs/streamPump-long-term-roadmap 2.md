...
| 2026-05-16 | Page `/goal` pass | Audited `/login`. Added a dev/demo-only readiness banner that labels the page as mixed live wallet/email auth plus preview social/local fallback, and moved the login panel's remaining hardcoded Chinese labels/status text onto the existing i18n dictionary so English mode is truthful on this page. | `npm run build --prefix app` passed. Browser smoke on `http://localhost:3000/login` with English locale confirmed the readiness banner, English login controls, terms copy, and no remaining login-page Chinese strings from the previous hardcoded set. `git diff --check` passed. | Production OAuth/Web3Auth verification and managed wallet mapping remain future Phase 2 work; this pass intentionally did not change auth behavior. Protected local files remain dirty/untracked and excluded. | Audit `/demo` next for demo-path copy and readiness claims. |
