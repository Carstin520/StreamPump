...
Current target: code-verified invite-only Pilot candidate on Solana devnet/test-USDC — not deployed production, not live, no real funds. **H0, H1, and H2 human gates are approved.** **P3 pilot recovery + readiness-gate work is implemented and verified locally only** — not deployed, no migrations applied, no live/real-credential smoke. **P3 has NOT yet had its independent Fable 5 review or human review node H3**; do not state either passed. Next gate: **Fable 5 review of P3, then human review node H3** (see Progress Ledger, 2026-07-12).
...
## Post-Pilot Code-State Inventory (NOT current availability)

> This table is a **post-Pilot backlog inventory of what exists in the codebase**, not a statement of current Pilot availability and not the current execution plan. For what the current Pilot actually exposes, the authoritative source is **Current Pilot Execution Plan** below. Nothing here is deployed, live, or handling real funds; everything runs on devnet / test-USDC only. The "Code-state label" column describes how far a code path has been built; the "Pilot availability" column is the binding statement of whether it is open to Pilot users. Readiness label vocabulary comes from `docs/product-readiness-phase-0.md`.

| Area | Code-state label (backlog, not availability) | Pilot availability (binding) | Remaining backlog work |
| --- | --- | --- | --- |
...
The current target is a **code-verified invite-only Pilot candidate on Solana devnet / test-USDC — not deployed production, not live, no real funds.** This section is the only present execution plan. The "Next Behavior" and staged "Demo To Production Plan" sections below describe the longer-term product target and are **post-Pilot backlog**, not current P1 work; nothing in them is authorized until its own later human (H) gate opens and its audit/legal/provider prerequisites are met.

**Present P1 work — the only in-scope lane:** harden and verify the invite-only **external-wallet** corridor and nothing else:

- external-wallet challenge/verify auth where the invite (allowlist) check runs only after a valid signature and the challenge is uniformly shaped and stateless;
...
- managed/email/social/provider identity and public managed execution (Stage 1 below) — needs production OAuth/Web3Auth/passkey verification, custody/KMS, and legal review;
...
**Remaining blockers before a real Pilot launch:** a real dedicated devnet RPC, a decided Pilot test-USDC mint, a complete production IDL artifact/package, a real deployment chain preflight, a deployed-corridor smoke, and external security audit + legal review. Do not present any closed lane, or the "production corridor" framing below, as current Pilot availability.
