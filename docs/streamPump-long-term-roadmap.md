...
| --- | --- | --- | --- | --- |
| 2026-06-14 | Toolchain modernization and legacy cleanup | Updated the app toolchain to Next 16.2.7, React 19.2.7, Tailwind 4.3.0, TypeScript 6.0.3, and ESLint 9; added `app/eslint.config.mjs` and Tailwind 4 PostCSS wiring; updated backend tests for Chai 6 ESM compatibility through `backend/tests/chai-cjs-compat.mjs`; removed detached legacy frontend helpers and backend oracle/settlement scaffolds; fixed a React 19 nullable ref issue in `/onboarding`. | Recorder evidence: `git branch --show-current` returned `codex/post-deadline-phase-0`; latest commit remained `f57f174`; `git diff --stat --find-renames` showed the tooling/cleanup diff; reference search found no active source references to removed helpers; `git diff --check`, `npm run build --prefix backend`, `npm run test:backend`, `npm run lint --prefix app`, and `npm run build --prefix app` passed. The first app build attempt failed inside the sandbox with a Turbopack process/port-binding permission error, then passed outside the sandbox. | No readiness promotion. No route/API product behavior, campaign economics, Anchor semantics, SPUMP transferability, or settlement truth model changed. Browser smoke was not run; production promotion still needs deployed Vercel/Render/Neon/R2/Mux smoke, operator visibility, and Track3 merchant/reconciliation integration. |
