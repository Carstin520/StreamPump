...
Current target: controlled technical Pilot verification on Solana devnet/test-USDC — invite-only, external-wallet-first, Track1-only, no real funds, and not a formal production launch. **H0–H4 are approved; P4 M2–M6 and allowlist cleanup are complete on backend release `88c0deb`. P5 is authorized for bounded Pilot operations/reliability/security/observability hardening and must stop at H5.** P6 is not authorized.
...
The current target is a **controlled technical Pilot on Solana devnet / test-USDC — invite-only, external-wallet-first, Track 1 only, no real funds, and not a formal production launch.** The fixed devnet program is upgraded, Neon has 26 applied migrations, Render (`dep-d9auio7lk1mc73c4r18g`) is live at backend `88c0debad6ecb7eacfe9e24793951f3794353f4c`, and Vercel Production remains at frontend `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. M6 completed through real R2/Mux/feed/proposal proof, exactly-once manual Track 1 settlement/replay, and allowlist cleanup to the sole human-approved external wallet. Track 2/3 and automatic settlement remain off. H4 was explicitly approved on 2026-07-14. P5 is now limited to the exact hardening scope below; P6 and every closed lane remain unauthorized.
...
| --- | --- | --- | --- | --- |
| Note | Ledger interpretation | Each row is a point-in-time snapshot. The newest row supersedes older rows' "current" or "next gate" wording. | Use the newest P5 row for current evidence. | Stop at H5 human review after the P5 Fable gate. |
