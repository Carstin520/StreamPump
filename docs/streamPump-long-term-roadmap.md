...
Current target: controlled technical Pilot verification on Solana devnet/test-USDC — invite-only, external-wallet-first, Track1-only, no real funds, and not a formal production launch. **H0–H3 are approved. P4 M2–M5 completed; M6 completed through the disposable corridor and exactly-once manual Track 1 settlement/replay on backend release `67ec60c`.** Allowlist restoration is blocked because no non-disposable pre-M6 baseline exists; H4 is pending. P5/P6 are not authorized.
...
The current target is a **controlled technical Pilot on Solana devnet / test-USDC — invite-only, external-wallet-first, Track 1 only, no real funds, and not a formal production launch.** The fixed devnet program is upgraded, Neon has 26 applied migrations, Render (`dep-d9at6enavr4c73b0rc20`) is live at backend commit `67ec60c7679aca2d7adad24780ae043370c426e3`, and Vercel Production remains at frontend commit `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. M6 completed through real R2/Mux/feed/proposal proof and manual Track 1 settlement plus same-key replay. The creator received exactly 1,000,000 raw test-USDC once; Track 2/3 and automatic settlement remained off. The disposable allowlist cannot yet be restored because no non-disposable pre-M6 baseline exists; H4 is pending. This section is the only present execution plan. The "Next Behavior" and staged "Demo To Production Plan" sections below are **post-Pilot backlog**; P5/P6 and all closed lanes remain unauthorized.
...
| --- | --- | --- | --- | --- |
| Note | Ledger interpretation | Each row is a point-in-time snapshot. The newest dated row supersedes older rows' "current" or "next gate" wording; do not read the 2026-07-13 M2–M5 row as current M6 status. | Use the 2026-07-14 M6 row for current evidence. | H4 remains pending as stated there. |
