...
Current target: code-verified invite-only Pilot candidate on Solana devnet/test-USDC — not deployed production, not live, no real funds. **H0 and H1 human gates are approved; P2 corridor-truth work (`d78815b..2e5130c`) is now a local review candidate.** Next gate: fixed P2 range → independent **Fable 5** review → close any blocker/major and rerun → **human review node H2** (Fable and H2 have not passed) (see Progress Ledger, 2026-07-12).
