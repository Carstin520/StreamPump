...
Current target: controlled technical Pilot verification on Solana devnet/test-USDC — invite-only, external-wallet-first, Track1-only, no real funds, and not a formal production launch. **H0–H3 are approved; P4 M2–M6 and allowlist cleanup are complete on backend release `88c0deb`. H4 is ready for human review but not approved.** P5/P6 are not authorized.
...
| Auth entry | `/login` attempts email OTP and wallet challenge APIs; preview social/local fallback is labeled in dev/demo mode. A server-verified Google/Apple authorization-code path is implemented locally behind the backend `SOCIAL_AUTH_ENABLED` runtime gate; the login page no longer requires a separate Vercel build-time flag to expose those real provider entries. Provider dashboards, secrets, custody review, deployment, and a later human gate are still required. |
...
The current target is a **controlled technical Pilot on Solana devnet / test-USDC — invite-only, external-wallet-first, Track 1 only, no real funds, and not a formal production launch.** The fixed devnet program is upgraded, Neon has 26 applied migrations, Render (`dep-d9auio7lk1mc73c4r18g`) is live at backend `88c0debad6ecb7eacfe9e24793951f3794353f4c`, and Vercel Production remains at frontend `097e9805b197398ae1c04cf5bf84f1044b3b2f19`. M6 completed through real R2/Mux/feed/proposal proof, exactly-once manual Track 1 settlement/replay, and allowlist cleanup to the sole human-approved external wallet. Track 2/3 and automatic settlement remain off. H4 is ready for human review, not approved. This section is the only present execution plan. The "Next Behavior" and staged "Demo To Production Plan" sections below are **post-Pilot backlog**; P5/P6 and all closed lanes remain unauthorized.

**Current P4/H4 lane — no scope expansion:** preserve and verify the invite-only **external-wallet** corridor and nothing else:
...
| --- | --- | --- | --- | --- |
| Note | Ledger interpretation | Each row is a point-in-time snapshot. The newest row supersedes older rows' "current" or "next gate" wording. | Use the H4 cleanup row for current evidence. | Stop at H4 human review. |
