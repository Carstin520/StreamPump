...
| 2026-07-15 | Permanent social-auth enablement | The retired `SOCIAL_AUTH_ENABLED` value is now ignored just like the old invite variables, preventing a stale Render `false` from silently closing Google/Apple registration. Google and Apple are always enabled; production startup requires exact frontend origins, complete provider settings, and a valid managed-wallet encryption key. `/health` is permanently `PUBLIC_SOCIAL_PILOT`. Optional wallet login and the independent operator-publication gate remain unchanged; no withdrawal or financial lane was opened. | Backend build PASS; focused safety/social tests **35/35** PASS; full backend **243/243** PASS; app production build PASS; deployment-verifier self-test PASS; `git diff --check` PASS. | Merge and verify both live provider starts return authorization URLs instead of `SOCIAL_AUTH_DISABLED`. |
