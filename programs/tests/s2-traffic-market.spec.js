...
describe("streampump-core S2 traffic market", function () {
    ...
    before(async () => {
        ...
        (0, chai_1.expect)(upgraded.level).to.equal(2);
        // Ensure creator status is S2 active
        // 确保创作者状态为 S2 活跃
        (0, chai_1.expect)(ctx.enumKey(upgraded.status)).to.equal("s2Active");
    });
    // =====================================================================
    ...
    it("settles track1 base + track2 success and allows fan claim", async () => {
        // ----- Configuration / 配置 -----
        const track1Base = 100000n; // Fixed base pay / 固定基础报酬
        const track2Budget = 1000000n; // Performance budget / 绩效预算
        const track3Budget = 300000n; // CPS budget / CPS 预算
        const track2Target = 1000n; // Target metric value / 目标指标值
        const stakeAmount = 200000n; // Fan's SPUMP endorsement / 粉丝的 SPUMP 背书量
        ...
        // 在操作前快照余额，用于后续断言
        const fanSpumpBefore = await ctx.tokenAmount(ctx.fanASpumpAta, spl_token_1.TOKEN_2022_PROGRAM_ID);
        const creatorUsdcBeforeBase = await ctx.tokenAmount(ctx.creatorS2UsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        ...
        await ctx.program.methods
            ...
            spumpTokenProgram: spl_token_1.TOKEN_2022_PROGRAM_ID,
        ...
        const creatorUsdcAfterBase = await ctx.tokenAmount(ctx.creatorS2UsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        (0, chai_1.expect)(creatorUsdcAfterBase - creatorUsdcBeforeBase).to.equal(track1Base);
        // ----- Verify Track 1 is one-time-only (replay protection) -----
        // ----- 验证 Track 1 是一次性的（防重放保护） -----
        await ctx.expectAnchorError(() => ctx.program.methods
            .settleTrack1Base()
            .accounts({
            oracle: ctx.oracle.publicKey,
            protocolConfig: ctx.protocolConfig,
        ...
        const sponsorUsdcBeforeSettle = await ctx.tokenAmount(ctx.sponsorAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        await ctx.program.methods
        ...
        const sponsorUsdcAfterSettle = await ctx.tokenAmount(ctx.sponsorAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        (0, chai_1.expect)(sponsorUsdcAfterSettle - sponsorUsdcBeforeSettle).to.equal(200000n);
        ...
        (0, chai_1.expect)(ctx.enumKey(proposalAfterSettle.status)).to.equal("resolvedSuccess");
        // Assert: track2UsdcDeposited reflects the fan pool share (160,000)
        // 断言：track2UsdcDeposited 反映粉丝池份额（160,000）
        (0, chai_1.expect)(proposalAfterSettle.track2UsdcDeposited.toString()).to.equal("160000");
        // ----- Fan claims endorsement reward: SPUMP principal + USDC share -----
        // ----- 粉丝领取背书奖励：SPUMP 本金 + USDC 份额 -----
        const fanUsdcBeforeClaim = await ctx.tokenAmount(ctx.fanAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        await ctx.program.methods
            .claimEndorsement()
            .accounts({
            user: ctx.fanA.publicKey,
            ...
            proposalUsdcVault,
            usdcTokenProgram: spl_token_1.TOKEN_PROGRAM_ID,
        })
            .rpc();
        const fanSpumpAfterClaim = await ctx.tokenAmount(ctx.fanASpumpAta, spl_token_1.TOKEN_2022_PROGRAM_ID);
        const fanUsdcAfterClaim = await ctx.tokenAmount(ctx.fanAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        // Assert: fan gets 100% SPUMP principal minted back (successful outcome)
        // 断言：粉丝获得 100% SPUMP 本金铸回（成功结果）
        (0, chai_1.expect)(fanSpumpAfterClaim).to.equal(fanSpumpBefore);
        // Assert: fan gets 160,000 USDC (entire Track 2 fan pool, as sole endorser)
        // 断言：粉丝获得 160,000 USDC（整个 Track 2 粉丝池，作为唯一背书者）
        (0, chai_1.expect)(fanUsdcAfterClaim - fanUsdcBeforeClaim).to.equal(160000n);
        // Assert: endorsement position is marked as claimed (prevents double-claim)
        // 断言：背书仓位被标记为已领取（防止重复领取）
        const positionAfterClaim = await ctx.program.account.endorsementPosition.fetch(endorsementPosition);
        (0, chai_1.expect)(positionAfterClaim.claimed).to.equal(true);
    });
    // =====================================================================
    // TEST 2: Track 2 failure → 95% SPUMP refund (5% slash)
    // 测试 2：Track 2 失败 → 95% SPUMP 退还（5% 罚没）
    ...
    it("settles track2 fail branch and returns 95% SPUMP on claim", async () => {
        // ----- Configuration / 配置 -----
        const track1Base = 50000n;
        const track2Budget = 500000n;
        const track3Budget = 200000n;
        const track2Target = 1000n;
        const stakeAmount = 120000n; // Fan stakes 120,000 SPUMP / 粉丝质押 120,000 SPUMP
        const { creatorProfile, proposal, proposalUsdcVault, deadline } = await ctx.createFundedProposal({
            creator: ctx.creatorS2,
            sponsor: ctx.sponsorA,
            track1Base,
        ...
        const proposalAfterSettle = await ctx.program.account.proposal.fetch(proposal);
        (0, chai_1.expect)(ctx.enumKey(proposalAfterSettle.status)).to.equal("resolvedFail");
        // Assert: track2UsdcDeposited reset to 0 (everything refunded to sponsor)
        // 断言：track2UsdcDeposited 重置为 0（全部退还给赞助商）
        (0, chai_1.expect)(proposalAfterSettle.track2UsdcDeposited.toNumber()).to.equal(0);
        ...
        const expectedSlash = (stakeAmount * 500n) / 10000n;
        // Assert: fan gets back original balance minus the slash
        // 断言：粉丝取回原始余额减去罚没部分
        (0, chai_1.expect)(fanSpumpAfterClaim).to.equal(fanSpumpBefore - expectedSlash);
        // Assert: no USDC payout on failure
        // 断言：失败时无 USDC 支付
        (0, chai_1.expect)(fanUsdcAfterClaim - fanUsdcBeforeClaim).to.equal(0n);
    });
    // =====================================================================
    ...
    it("refunds the track2 fan pool to sponsor when nobody endorsed", async () => {
        const track1Base = 40000n;
        const track2Budget = 1000000n;
        const track3Budget = 150000n;
        const track2Target = 1000n;
        const { creatorProfile, proposal, proposalUsdcVault, deadline } = await ctx.createFundedProposal({
            creator: ctx.creatorS2,
            sponsor: ctx.sponsorA,
        ...
        (0, chai_1.expect)(sponsorUsdcAfterSettle - sponsorUsdcBeforeSettle).to.equal(360000n);
        (0, chai_1.expect)(proposalAfterSettle.track2UsdcDeposited.toString()).to.equal("0");
        (0, chai_1.expect)(ctx.enumKey(proposalAfterSettle.status)).to.equal("resolvedSuccess");
    });
    // =====================================================================
    ...
    it("settles track3 CPS and splits approved payout/refund correctly", async () => {
        const track1Base = 30000n;
        const track2Budget = 200000n;
        const track3Budget = 600000n; // Total CPS budget / 总 CPS 预算
        const approvedCpsPayout = 250000n; // Oracle-approved commission / 预言机批准的佣金
        const { creatorProfile, proposal, proposalUsdcVault, deadline } = await ctx.createFundedProposal({
            creator: ctx.creatorS2,
            sponsor: ctx.sponsorA,
            track1Base,
        ...
        // 在结算前快照余额
        const creatorUsdcBefore = await ctx.tokenAmount(ctx.creatorS2UsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        const sponsorUsdcBefore = await ctx.tokenAmount(ctx.sponsorAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        ...
        await ctx.program.methods
            ...
            .signers([ctx.oracle])
            .rpc();
        const creatorUsdcAfter = await ctx.tokenAmount(ctx.creatorS2UsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        const sponsorUsdcAfter = await ctx.tokenAmount(ctx.sponsorAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        ...
        (0, chai_1.expect)(creatorUsdcAfter - creatorUsdcBefore).to.equal(approvedCpsPayout);
        // Assert: sponsor received the unused budget back
        // 断言：赞助商收回了未使用的预算
        (0, chai_1.expect)(sponsorUsdcAfter - sponsorUsdcBefore).to.equal(expectedRefund);
        ...
        (0, chai_1.expect)(proposalAfter.track3CpsPayout?.toString()).to.equal(approvedCpsPayout.toString());
        (0, chai_1.expect)(proposalAfter.track3SettledAt.toNumber()).to.be.greaterThan(0);
    });
    // =====================================================================
    ...
    it("emergency_void refunds vault and still allows 100% SPUMP claim", async () => {
        const track1Base = 70000n;
        const track2Budget = 400000n;
        const track3Budget = 500000n;
        const track2Target = 1000n;
        const stakeAmount = 90000n; // Fan stakes 90,000 SPUMP / 粉丝质押 90,000 SPUMP
        ...
        // ----- 管理员触发紧急作废 -----
        const sponsorUsdcBeforeVoid = await ctx.tokenAmount(ctx.sponsorAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        const vaultBeforeVoid = await ctx.tokenAmount(proposalUsdcVault, spl_token_1.TOKEN_PROGRAM_ID);
        await ctx.program.methods
            .emergencyVoid()
            .accounts({
        ...
        const sponsorUsdcAfterVoid = await ctx.tokenAmount(ctx.sponsorAUsdcAta, spl_token_1.TOKEN_PROGRAM_ID);
        (0, chai_1.expect)(sponsorUsdcAfterVoid - sponsorUsdcBeforeVoid).to.equal(vaultBeforeVoid);
        ...
        (0, chai_1.expect)(ctx.enumKey(proposalAfterVoid.status)).to.equal("voided");
        (0, chai_1.expect)(proposalAfterVoid.track1Claimed).to.equal(true); // Blocked from re-claim / 阻止重复领取
        (0, chai_1.expect)(proposalAfterVoid.track2UsdcDeposited.toNumber()).to.equal(0);
        (0, chai_1.expect)(proposalAfterVoid.track3UsdcDeposited.toNumber()).to.equal(0);
