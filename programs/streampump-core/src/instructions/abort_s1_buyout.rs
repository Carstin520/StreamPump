// EN: Abort an accepted S1 buyout when every S1 holder has rage-quit.
// ZH: 当所有 S1 持有人都已 rage-quit 后，中止已接受的 S1 买断。
use anchor_lang::prelude::*;
use anchor_spl::token::{self, CloseAccount, Mint, Token, TokenAccount, Transfer};

use crate::{
    errors::StreamPumpError,
    events::S1BuyoutAborted,
    state::{CreatorProfile, CreatorStatus, ProtocolConfig, S1BuyoutOffer, S1BuyoutState},
};

#[derive(Accounts)]
pub struct AbortS1Buyout<'info> {
    #[account(mut)]
...
pub(crate) fn handler(ctx: Context<AbortS1Buyout>) -> Result<()> {
    ...
    require!(
        ctx.accounts.buyout_offer.usdc_amount == ctx.accounts.s1_buyout_state.usdc_deposited,
    ...
    ctx.accounts.creator_profile.updated_at = now;
