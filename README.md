...
**Not claimed** (do not represent these as done): independent third-party publication verification; program-side allowlist enforcement; security audit; public/real-funds production launch; real funds.
...
**Next gate:** H4 was explicitly approved on 2026-07-14. P5 is freezing one bounded control-plane hardening candidate, running risk-corresponding checks, and submitting only the uncovered `34f3d96..candidate` range to Fable 5. After a 0 blocker/major verdict, work stops at **H5**. P6 does not begin without separate H5 approval.

**Remaining blockers before any public or real-funds launch:** external security audit; legal/token-classification review; production policy and jurisdiction/KYC decisions; and separate approval for every currently closed lane. H6 would still not waive these blockers.
...
Present priority: **P5 bounded Pilot operations/reliability/security/observability hardening**, still invite-only, devnet/test-USDC, external-wallet-first, Track1-only, manual/operator-only, and no real funds. P4 is complete and H4 is approved; P5 must pass its exact-range Fable 5 gate and stop at H5.
...
- **P5 gate sequence (current):** freeze a bounded hardening candidate from accepted boundary `34f3d96`, run only risk-corresponding verification, review the uncovered range once with Fable 5, close blocker/major findings, then stop at H5. No closed lane or readiness promotion is part of P5.
...
- **P3 gate sequence (historical, complete):** the initial Fable 5 review found 2 majors; both were fixed in `96e9075`, the fixed range `84c3415..d783301` passed re-review, and H3 was approved on 2026-07-12.
- Preserve the P4 dedicated devnet RPC, run-scoped test-USDC freeze, packaged IDL, rollback evidence, deployment identities, and controlled corridor/Track 1 proof without routinely rerunning the disposable smoke.
